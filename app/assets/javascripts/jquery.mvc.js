;(function($) {
  ...
  $.Controller = (function() {
    function Controller(name, obj, subs, ancestorClass) {
      ...
      var defaultEvents = "change click contextmenu dblclick keydown keyup keypress mousedown mousemove mouseout mouseover mouseup reset resize scroll select submit focus blur focusin focusout mouseenter mouseleave draginit dragstart drag dragend".split(" ");
        ...
        this.super_call = function(meth_name, args) {
          ...
          while((i < all_methods.length) && not_found) {
            ...
            if((!sub_event && (meth[0] == meth_name)) || (sub_event && ((meth[0] == meth_name[0]) && (meth[1] == meth_name[1])))) {
              meth[meth.length - 1].apply(this, args);
              not_found = false;
            }
            i += 1;
          }
