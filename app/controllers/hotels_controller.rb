class HotelsController < ApplicationController
  ...
  def complete
    data = api.complete(params[:term])
    render :json => data
  end

  def index
    @hotels_search = HotelsSearch.new(:rooms=>[HotelRoom.new()])
    render :layout =>"map"
  end

  def get_advanced_form
    @count = (params[:count].to_i - 1).times.collect{|i| i+1 }
    html   = render_to_string :partial =>"hotels/search_include"
  ...
  def search
    @search       = HotelsSearch.new(params[:hotels_search])
    ...
    session_id  = Digest::MD5.hexdigest(@search.to_json)
    lat_lng     = api.get_location(@search.city , @search.place_code,Digest::MD5.hexdigest("#{session_id}#{request.remote_ip}"))
    
    api.hotels_list(@search.to_api_hash,Digest::MD5.hexdigest("#{session_id}#{request.remote_ip}"))
    ...
    if api.error.nil? and @results.size > 0
      ...
      interval  = (@search.departure - @search.arrival).to_i unless  @search.nil?
      ...
      render :json => {:success => true, :data=>@results.map{|h| {:id=>h[:id],:lat=>h[:lat],:lng=>h[:lng],:name=>h[:name]}}, :html => render_to_string(:partial => "hotels/list")}
    else
