...
class Api::Cars
  ...
  def search_cars_request search
    ...
    pick_date = (search.pick_up.date + '-'+search.pick_up.time).split('-')
    drop_date = (search.dropp_off.date + '-'+search.dropp_off.time ).split('-')
    ...
    result =
      @xml.SearchRQ(@prefered){
        ...
        @xml.PickUp{
            ...
            @xml.Date(:year=>pick_date[0],:month=>pick_date[1],:day=>pick_date[2],:hour=>pick_date[3],:minute=>pick_date[4])
        }
        @xml.DropOff{
            @xml.Location(:id=>drop_lock)
            @xml.Date(:year=>drop_date[0],:month=>drop_date[1],:day=>drop_date[2],:hour=>drop_date[3],:minute=>drop_date[4])
        }
        @xml.DriverAge(age)
      }
  end
  ...
  def booking_request booking ,card
    pick_date = booking.pick_up.date.split('-')
    drop_date = booking.dropp_off.date.split('-')
    ...
    if type == 'PickUpOpenTimeRQ'
      result =  
        @xml.PickUpOpenTimeRQ(@prefered){
          ...
          @xml.Date(:year=>date[0],:month=>date[1],:day=>date[2])
        }
    else
